...
df = pd.read_csv('5-Projeto-Dash-de-vendas-call_center/projeto_completo/dataset_asimov.csv')
df_cru = df.copy()
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dbc.Row(
                        dbc.Col(
                            html.Legend('Top Consultores por Equipe', style={"fontSize": "18px", "textAlign": "center"})
                        )
                    ),
                    dbc.Row([
                        dbc.Col([
                ...
                dbc.CardBody([
                    dbc.Row(
                        dbc.Col([
                            ...
                            html.Div(id='month-select', style={'text-align': 'center', 'margin-top': '22px'}, className='dbc')
                        ])
                    )
                ])
            ], style=tab_card)
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.Div(id='team-select', style={'text-align': 'center', 'margin-top': '58px'}, className='dbc')
                ])
            ], style=tab_card)
        ], sm=12, lg=2),
    ], className='g-2 my-auto', style={'margin-top': '7px'})
...
def graph1(month, toggle):
    ...
    fig1.update_layout(main_config, height=143, template=template)
    fig2.update_layout(main_config, height=143, template=template, showlegend=False)

    select = html.H3(convert_to_text(month))
...
def graph3(team, toggle):
    ...
    fig3.update_layout(main_config, height=175, template=template)
    return fig3
...
def graph4(team, toggle):
    ...
    fig4.update_layout(main_config, height=175, template=template)
    return fig4
...
def graph5(month, toggle):
    ...
    fig5.add_trace(go.Indicator(mode='number+delta',
        title = {"text": f"<span style='font-size:20px; font-weight:700'>{df_5['Consultor'].iloc[0]} - Top Consultant</span><br><span style='font-size:18px'>Em vendas - em relação a média</span><br>"},
        value = df_5['Valor Pago'].iloc[0],
        number = {'prefix': "R$", "font": {"size": 60}},
    ...
    fig6.add_trace(go.Indicator(mode='number+delta',
        title = {"text": f"<span style='font-size:20px; font-weight:700'>{df_6['Equipe'].iloc[0]} - Top Team</span><br><span style='font-size:18px'>Em vendas - em relação a média</span><br>"},
        value = df_6['Valor Pago'].iloc[0],
        number = {'prefix': "R$", "font": {"size": 60}},
        delta = {'relative': True, 'valueformat': '.1%', 'reference': df_6['Valor Pago'].mean()}
    ))

    fig5.update_layout(main_config, height=175, template=template)
    fig6.update_layout(main_config, height=175, template=template)
...
def graph7(toggle):
    ...
    fig7.update_layout(main_config, yaxis={'title': None}, xaxis={'title': None}, height=207, template=template)
...
def graph8(month, toggle):
    ...
    fig8.update_layout(main_config, height=424, template=template)
    return fig8
...
def graph9(month, team, toggle):
    ...
    fig9.update_layout(main_config, height=140, template=template, showlegend=False)
    return fig9
...
def graph10(team, toggle):
    ...
    fig10.update_layout(main_config, height=168, template=template, showlegend=False)
    return fig10
...
def graph11(month, team, toggle):
    ...
    fig11.add_trace(go.Indicator(mode='number',
        title = {"text": f"<span style='font-size:38px; font-weight:700'>Valor Total</span><br><span style='font-size:18px'>Em Reais</span><br>"},
        value = df_11['Valor Pago'].sum(),
        number = {'prefix': "R$", "font": {"size": 70}}
    ))

    fig11.update_layout(main_config, height=205, template=template)
    select = html.H4("Todas Equipes") if team == 0 else html.H4(team)
